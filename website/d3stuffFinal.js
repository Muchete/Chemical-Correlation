...
var rrr = [];
var n = 0;
var corrNames = ["very converse curve","slightly converse curve","neutral correlation","slightly similar curve","very similar curve"];
var allGenres = ["COUNTRY", "METAL", "R_B_SOUL", "NEW_AGE", "REGGAE_SKA", "HIP_HOP_RAP", "JAZZ", "ALTERNATIVE_INDIE", "FOLK", "CHRISTIAN_GOSPEL", "ROCK", "LATIN", "WORLD", "CLASSICAL", "DANCE_ELECTRONIC", "BLUES", "POP"];
var allDrugs = ["Cannabis", "LSD", "MDMA", "Amphetamines", "Cocaine", "Heroin", "Ketamine", "Methamphetamine", "Mushrooms"];
...
// parse cor Data
// var parseCor = d3.scaleLinear().domain([-1, 1]).range([0, 100]);
var parseCor = d3.scaleLinear().domain([-1, 1]).range([0, corrNames.length]);
...
d3.json("data/ULTIMATE_DATA.json", function(error, data) {
    ...
    var x = d3.scaleTime()
        .range([0, width])
        .domain(d3.extent(data.drugData[0].popularity, function(d) { return d.year; }));
    ...
    corBox.selectAll('.corVal')
        .data(data.corData.filter(function(d) { return d.genre == currentGenre })
            .filter(function(d) { return d.drug == currentDrug }))
        .enter()
        .append('div')
        .attr('class', 'corVal')
        .text(function(d) {
    ...
    drawCurve(data.drugData.filter(function(d) { return d.tag == currentDrug; }), "drug");
    drawCurve(data.musicData.filter(function(d) { return d.tag == currentGenre; }), "genre");
    drawAxis();
    ...
    function update() {
        var currentDrugData = data.drugData.filter(function(d) { return d.tag == currentDrug; });
        var currentMusicData = data.musicData.filter(function(d) { return d.tag == currentGenre; });

        updateCurve(currentDrugData, "drug");
        updateCurve(currentMusicData, "genre");

        //update correlation number
        updateCorNumber(data.corData.filter(function(d) { return d.genre == currentGenre })
            .filter(function(d) { return d.drug == currentDrug }));
    ...
    function formatPopData(ar) {
        ar.forEach(function(d) {
            var max = d3.max(d.popularity, function(l){ return l.val });
    ...
    function updateCurve(dataSet, genreOrDrug) {
        ...
        var area = d3.area()
            ...
            .y0(function(d) { return y(d.normVal * -1); })
            // .y0(function(d) { return y(d.val * -1); })
            .y1(function(d) { return y(d.normVal); })
            // .y1(function(d) { return y(d.val); })
            .curve(d3.curveMonotoneX);

        svg.selectAll('g.' + genreOrDrug + ' path')
    ...
    $( "g.drug path" ).hover(function() {
        $("g.genre path").css('opacity', '.4');
    }, function() {
        $("g.genre path").css('opacity', '1');
    });

    $( "g.genre path" ).hover(function() {
        $("g.drug path").css('opacity', '.4');
    }, function() {
        $("g.drug path").css('opacity', '1');
    });
});
